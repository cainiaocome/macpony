...
When a client is replaced between polls, the server permits one bounded
recovery read after the original read ages out. This is deliberately capped at
one extra read and clears stale tags once the recovery read receives data, so
the client-recovery fix does not become an unbounded socket-read queue.
